...
- **B — `POST /v1/vpn/authorize` (per-connection pseudonymous credential
  ...
  Originally reused each device's `vpn_accounts` identity with an advisory
  `expires_at`; superseded by Sub-project B2
  (`docs/ADR/0003-ephemeral-managed-authorization.md`): per-hop credentials
  are now short-lived lease-pool slots whose `expires_at` is enforced by
  the node.
- **C — real interop testing against `tamara-next`'s built client: DONE**
  (`tamara-next/test/infrastructure/vpn_web_interop_test.dart`, real
  signed envelopes generated by this repo's `signRouteDirectory` verified
  against `tamara-next`'s actual `RouteDirectoryVerifier`). Found and
  fixed two real cross-repo bugs no unit test on either side alone could
  have caught: (1) `tamara-next` rejected an empty `routes` list as
  malformed, though that is this repo's real response until a node
  reports transport (`tamara-next` PR #25); (2) this repo sent
  `server_address` as a DNS hostname, but `tamara-next`'s client requires
  an IP literal for any non-chained hop (a deliberate anti-plaintext-DNS
  property) — fixed by using the already-existing but previously-unused
  `nodes.ip_address` column instead of `hostname` (this repo, PR #47).
  Sub-project A's "SHIPPED" status above already reflects this fix.
